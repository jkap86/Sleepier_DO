        ...
        ?.map(league => {
            ...
            if (recordType === 'Projected Record') {
                standings = league.rosters
                    .map(roster => {
                        const projection_roster = recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)
            ...
            const record = {
                wins: league.userRoster.settings.wins + ((recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)?.wins) || 0),
                losses: league.userRoster.settings.losses + ((recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)?.losses) || 0),
                ties: league.userRoster.settings.ties + ((recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)?.ties) || 0),
                fpts: parseFloat(league.userRoster.settings.fpts + '.' + league.userRoster.settings.fpts_decimal)
                    + ((recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)?.fpts) || 0),
                fpts_against: parseFloat(league.userRoster.settings.fpts_against + '.' + league.userRoster.settings.fpts_against_decimal)
                    + ((recordType === 'Projected Record' && getProjection(league.league_id, league.settings.playoff_week_start, lineupChecks, state)?.fpts_against) || 0)
            }
